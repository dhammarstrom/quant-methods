...
In the example below, we use `rnorm()` to simulate numbers from a normal distribution. Using the arguments `n = 10`, `mean = 0`, and `sd = 1`, we simulate randomly picking ten numbers from a distribution with a mean of 0 and a standard deviation of 1. These numbers are stored in a data frame that is assigned to an object that we have named `dat`.  

...
## An exercise in data wrangling and tables

In total 30 college students performed a heavy-resistance training protocol where training volume was constantly increased over six weeks. In [@RN2149], a part of the study, focusing on supplementation was reported. In [@RN2225], participants were divided into two clusters based on training responses and the authors aimed to answer the question what separates high- from low-responders to resistance training.

In this exercise we want to reproduce a big part of Table 1 in [@RN2225]. The Table as re-produced here can be seen below. See the original article for explanation of clusters. To select variables, see the data description in the `exscidata` package, the data set is called `hypertrophy`.

```{r, echo = FALSE, message = FALSE, warning = FALSE}
#| echo: false
#| message: false
#| warning: false
#| tbl-cap: "Baseline characteristics at PRE and back squat training volume between clusters"
...
#| eval: false
#| code-fold: true
#| code-summary: "A possble solution"
# load the data
data(hypertrophy)

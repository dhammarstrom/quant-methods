...
Consider the sampling distribution. This is an imaginary distribution of e.g. sample means gathered from repeated sampling from a population of numbers. Let's say that our null-hypothesis, H~0~, is that the mean of a population is zero. We draw a sample of $n=10$ and it turns out that it is not zero, it is about 1. We calculate the standard deviation of our sample, it is 2. 
...
It would look something like this (@fig-sampling-dist).
...
#| message: false
#| warning: false
#| fig-cap: "An imaginary sampling distribution"
...
#| message: false
#| warning: false
#| fig-cap: "An imaginary sampling distribution with a *t*-statistic."
#| label: fig-sampling-dist-tstat
...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  ...
  scale_y_continuous(breaks = NULL) +
        ...
        geom_vline(xintercept = (1-0) / (2/sqrt(10))) +
        annotate("richtext", 
                 x = 1.6, 
                 size = 2.8,
                 y = 0.12, 
...
#| warning: false
#| fig-cap: "An imaginary sampling distribution with a *t*-statistic."
#| label: fig-sampling-dist-critical
...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  ...
  scale_y_continuous(breaks = NULL) +
        ...
        geom_vline(xintercept = (1-0) / (2/sqrt(10))) +
        annotate("text", x = 1.75, y = 0.25, label = "The observed t", 
                 size = 2.8, angle = -90) +
...
#| message: false
#| warning: false
#| fig-cap: "An imaginary sampling distribution with a *t*-statistic and a two-tailed test agaionst the null hypothesis."
#| label: fig-sampling-dist-twotailed
...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  ...
  scale_y_continuous(breaks = NULL) +
        ...
        geom_vline(xintercept = (1-0) / (2/sqrt(10))) +
        annotate("text", x = 1.75, y = 0.2, label = "The observed t", angle = -90, 
                 size = 2.8) +
        
        annotate("text", x = 2.6, y = 0.12, label = "Area of more\nextreme values", 
                 size = 2.8) +
...
As the area under the curve in the blue area is `r round(p * 100, 2)`% of the distribution we may make a statement such as: "If the null-hypothesis is true, our observed value, or a value even more distant from 0 would appear `r round(p * 100, 2)`% of the times upon repeated sampling".
...
#| message: false
#| warning: false
#| fig-cap: "Results of differences between groups using a permutation (or reshuffeling) procedure. The black line represents the "
...
```
The above code calculates the proportion of mean differences greater than the observed difference that occurred when we re-shuffled the groups by chance. The number of total re-shuffles that are greater than our observation corresponds to `r round(p * 100, 1)`%. 
...
#| message: false
#| warning: false
#| fig-cap: "Results of differences between groups using a permutation (or reshuffeling) procedure. The black line represents the observed difference."
#| label: fig-permutation-test-onetail
...
$$H_A: \text{Strength} > \text{Endurance}$$
We can account for the fact that the endurance group could be stronger than the strength group with the figure below (@fig-permutation-test-twotail). Try to figure out what the code does and what has changed from above.
...
#| message: false
#| warning: false
#| fig-cap: "Results of differences between groups using a permutation (or reshuffeling) procedure. The black line represents the observed difference. More extreme permutations compared to our observed value are highlighted."
#| label: fig-permutation-test-twotail
...
p <- length(results[abs(results$m) > mean.difference,]) / 1000


```

The proportion of more extreme values, irrespective of direction, compared to our observed value is `r round(p * 100, 1)`%.
...
We also above state the level of &alpha; needed to reject the null hypothesis. This is our target error rate for rejecting the null-hypothesis in cases when we should not do so.
...
p <- false.positive / 10000 # should be ~ 5%
...
#| message: false
#| warning: false
#| fig-cap: "P-values from 10000 t-test of samples from a population where the null-hypothesis is true."
#| label: fig-null-hypothesis-p
...
1. The t-test suitable for a paired comparisons of two set of values is the paired-sample t-test. 
...
3. The p-value can be interpreted as the probability of obtaining a result as extreme or even more extreme than the observed value. The confidence interval is an interval containing the true population value in 95% of theoretical repeated studies. 

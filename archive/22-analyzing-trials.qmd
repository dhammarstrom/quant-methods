...
In a design where we have a Treatment group and a control group for comparison hypothesized outcomes can look something like in @fig-trial-design-fig.

```{r}
#| label: fig-trial-design-fig
#| fig-align: "center"
#| fig-width: 4
#| fig-height: 2.5
#| fig-cap: "Hypothesized values from a simple pre-post parallel-groups design" 
...
Another common scenario is that we expect progress in two different treatments groups as in @fig-trial-design-fig2.


```{r}
#| label: fig-trial-design-fig2
#| fig-width: 4
#| fig-height: 2.5
#| fig-cap: "Hypothesized values from a simple pre-post parallel-groups design including to different treatments"
#| fig-align: "center"
...
$$\text{outcome} = \beta_0 + \beta_1 x_\text{GroupB}$$
...
$$\text{Percentage difference} = (1-\operatorname{exp}(\text{estimate})) \times 100$$

If we want to express the 30RM group as a percentage of the 10RM group we could remove 1 from the equation:

$$\text{Percentage of 10RM} = \operatorname{exp}(\text{estimate}) \times 100$$
...
$$\text{Change} = \beta_0 + \beta_1 \times x_{\text{group}}$$

The extended, ANCOVA model can be written as

$$\text{Change} =  \beta_0 + \beta_1 \times x_\text{Baseline} + \beta_2 \times x_\text{Group}$$
